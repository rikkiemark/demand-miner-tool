...
- **allintitle 検索**: DataForSEO SERP API で `allintitle:キーワード` を検索し、タイトルにそのキーワードを含む記事の件数を取得。件数が少ないほど競合が少ない
- **時間指定検索**: 設定可能な日数フィルタ（デフォルト: 7日/30日/90日）で、各期間内に公開された記事の件数を取得。件数が少ないほど情報が古く、上位を奪いやすい。日数フィルタはプロファイルごとに自由に変更可能
...
| 任意 API | DataForSEO Google Organic SERP API（ランク判定に必要） |

> **Note:** ランク判定機能には DataForSEO の有料 API アカウントが必要です（Standard キュー: $0.0006/タスク、allintitle 検索は $0.003/タスク）。API キーがない場合でも、サジェスト取得機能（Route A / Route B）は動作します。
...
| 競合分析 | DataForSEO Google Organic SERP API (Standard) | allintitle 検索・時間指定検索（バッチ処理対応） |
...
│   ├── search_analyzer.py      # DataForSEO SERP API 連携
│   │                             - batch_analyze(): バッチ競合分析
...
└────────────────────┬────────────────────┘
                     ...
                     ▼
┌─────────────────────────────────────────┐
│ Step 4: 競合分析（DataForSEO バッチ処理）    │
│                                           │
│  4a. allintitle 検索タスクをバッチ投稿       │
│      （最大100件単位で task_post）           │
│      → ポーリング（約5分待機）              │
│      → 結果取得（task_get）                │
│                                           │
│  4b. 時間指定検索（設定可能な日数フィルタ）    │
│      検索順序を選択可能:                     │
│      ・long_to_short: 90日→30日→7日        │
│        （長期間で0件なら短期間はスキップ）     │
│      ・short_to_long: 7日→30日→90日        │
│        （短期間で記事ありなら即判定）          │
│      → 同様にバッチ投稿→ポーリング→結果取得   │
│                                           │
│  4c. ドメインマッチ確認（スナイパー判定用）    │
└────────────────────┬────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────┐
...
| タイムアウト | `dataforseo_post` | 30 | DataForSEO タスク投稿のタイムアウト（秒） |
| タイムアウト | `dataforseo_poll` | 300 | DataForSEO ポーリング全体のタイムアウト（秒） |
| タイムアウト | `dataforseo_get` | 30 | DataForSEO 結果取得のタイムアウト（秒） |
| タイムアウト | `trends_api` | 20 | トレンド API のタイムアウト（秒） |
| DataForSEO | `batch_size` | 100 | 1回のバッチ投稿で送るタスク数（最大100） |
| DataForSEO | `poll_interval` | 30 | ポーリング間隔（秒） |
| DataForSEO | `location_code` | 2392 | 検索対象の国コード（2392=日本） |
| DataForSEO | `language_code` | "ja" | 検索言語 |
| コスト管理 | `max_cost_per_run` | 10.0 | 1回の実行あたりのコスト上限（ドル、0=無制限） |
| コスト管理 | `allintitle_cost` | 0.003 | allintitle 検索1件あたりのコスト（ドル） |
| コスト管理 | `serp_cost` | 0.0006 | 通常 SERP 検索1件あたりのコスト（ドル） |
...
| 日付フィルタ | `date_filters` | 時間指定検索の日数リスト（デフォルト: [7, 30, 90]） |
| 検索順序 | `search_order` | 日付フィルタの実行順序（`long_to_short` / `short_to_long`） |
| ランク閾値 | `ranking.rank_s_days` | S ランク判定の日数閾値（デフォルト: 90日、date_filters と連動） |
| ランク閾値 | `ranking.rank_a_days` | A ランク判定の日数閾値（デフォルト: 30日、date_filters と連動） |
| ランク閾値 | `ranking.rank_b_days` | B ランク判定の日数閾値（デフォルト: 7日、date_filters と連動） |
...
ランク判定機能は DataForSEO の有料 API を使用します。コストを最適化するための仕組みが組み込まれています。

### コスト概算

| 処理 | 単価 | 例（1,000KW） | 備考 |
|------|------|--------------|------|
| allintitle 検索 | $0.003/件 | $3.00 | 全キーワード分 |
| 時間指定検索（1期間） | $0.0006/件 | $0.60 | date_filters の期間数分 |
| **合計（3期間の場合）** | - | **約$4.80** | 早期打ち切りでさらに削減可能 |

### コスト最適化の仕組み

1. **検索順序の最適化（search_order）**
   - `long_to_short`: 90日で0件なら30日/7日の検索をスキップ（コスト削減優先）
   - `short_to_long`: 7日で記事があれば即 C 判定で以降スキップ（速度優先）

2. **Smart TTL キャッシュ**: 一度分析したキーワードは次回実行時にキャッシュから取得（API コスト $0）

3. **バッチ処理**: 最大100件単位でまとめて投稿し、API オーバーヘッドを削減

4. **コスト上限設定**: `max_cost_per_run` で1回の実行あたりの上限を設定可能。上限に達すると残りのキーワードはスキップ
...
| GUI（Gradio） | 稼働中 |
| キャッシュ機能 | 稼働中 |
| ランク判定（DataForSEO SERP API） | 実装済み |
