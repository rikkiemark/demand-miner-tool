...
**従来の総当たり方式**では、10個の種キーワード × 10個の感情ワード × 全サジェスト = 約5,400回の API 呼び出しが必要でした。スマート再帰掘りでは、**感情キーワードが含まれる有望な枝だけを選択的に深掘り**することで、API コール数を約700〜1,000回に削減しています。

#### 動作の流れ

```
Step 1: 種キーワード「彼氏」で Google サジェストを取得
        → [彼氏 既読無視, 彼氏 プレゼント, 彼氏 蛙化, 彼氏 誕生日, ...]

Step 2: 感情キーワード（既読無視, 蛙化, 辛い 等）でフィルタリング
        → 「彼氏 既読無視」「彼氏 蛙化」は感情ワードを含む → 深掘り対象
        → 「彼氏 プレゼント」「彼氏 誕生日」は含まない → スキップ（API 節約）

Step 3: 深掘り対象を次の階層で再度サジェスト取得
        → 「彼氏 既読無視」→ [彼氏 既読無視 3日, 彼氏 既読無視 1週間, ...]
...
```
従来（総当たり）:
  10種 × 10感情 × 各サジェスト取得 = 約5,400 API コール

スマート再帰掘り:
  10種 → サジェスト取得（10回）
    → 感情フィルター通過分のみ深掘り（約70回）
      → さらに深掘り（約200回）
...
```
例: トレンド「大谷翔平」× 補助ワード「結婚」× 感情ワード「辛い」
  → 「大谷翔平 結婚 辛い」で Google サジェストを取得
```

トレンドに乗った一時的な需要を、感情系の切り口で記事化する戦略に向いています。
...
ジャンル（恋愛、AI技術、料理 等）ごとに**ワード群と分析設定を分離管理**する仕組みです。
...
│   ├── settings.yaml
│   └── data/
│       ├── seeds.txt      # ChatGPT, Claude, Gemini, ...
│       ├── emotions.txt   # 使えない, すごい, 怖い, ...
│       └── ...
...
```csv
rank,keyword,allintitle_count,route
SS,元彼 復縁 既読無視 3日,2,定番ルート（スマート再帰掘り）
S,彼氏 既読無視 3日,0,定番ルート（スマート再帰掘り）
A,元彼 蛙化 心理,3,定番ルート（スマート再帰掘り）
B,復縁 辛い LINE,8,トレンドルート（総当たり）
C,彼氏 プレゼント クリスマス,156,定番ルート（スマート再帰掘り）
```
